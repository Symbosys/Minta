import { Edit, RemoveRedEye, Save } from '@mui/icons-material'
import { Button, Card, Container, FormControl, Grid, InputLabel, MenuItem, Select, TextField } from '@mui/material'
import React from 'react'


function Profile() {

    const [edit, setEdit] = React.useState(false);  

  return (
      <Container maxWidth="sm" >
          <Card className="p-3 mt-5">
              <Grid container spacing={2}>
                  <Grid item xs={12} sm={4}>
                      <img className='img-fluid' src='data:image/jpeg;base64,' alt='Profile' />
                  </Grid>
                  <Grid item xs={12} sm={8}>
                      <TextField className="pb-2" label="Name" variant="filled" fullWidth value="Sudhanshu Kumar" disabled={!edit} />
                      <TextField className="pb-2" label="Email" variant="filled" fullWidth value="Sudhanshu@gmail.com" disabled={!edit} />
                      <TextField className="pb-2" label="Contact" variant="filled" fullWidth value="+91 85412456385" disabled={!edit} />
                      <FormControl className="pb-2" variant="filled" fullWidth>
                          <InputLabel>Gender</InputLabel>
                          <Select disabled={!edit}>
                              <MenuItem value="male">Male</MenuItem>
                              <MenuItem value="female">Female</MenuItem>
                              <MenuItem value="other">Other</MenuItem>
                          </Select>
                      </FormControl>

                  </Grid>
                  <Grid item xs={12} sm={9}>
                  </Grid>
                  <Grid item xs={12} sm={3}>
                      <Button endIcon={edit ? <Save /> : <Edit />} onClick={() => setEdit(!edit)}>{edit ? "Save" : "Edit"}</Button>

                  </Grid>

              </Grid>
          </Card>


          <Card className="p-3 mt-5">
              <Grid container spacing={2}>
                  <Grid item xs={12} >
                      <TextField label="Old Password" variant="filled" fullWidth type='password' />
                  </Grid>
                  <Grid item xs={12} >
                      <TextField label="New Password" variant="filled" fullWidth type='password' />
                  </Grid>
                  <Grid item xs={12} >
                      <TextField label="Confirm Password" variant="filled" fullWidth type='password' />
                  </Grid>
                  <Grid item xs={0} sm={7} >
                  </Grid>
                  <Grid item xs={12} sm={5} >

                      <Button endIcon={<RemoveRedEye />}>Change Password</Button>
                  </Grid>
              </Grid>
          </Card>
</Container>
  )
}

export default Profile
